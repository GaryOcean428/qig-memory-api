...
async function callMember(member, { system, prompt, maxOutputTokens }) {
  const attempt = async (model) => {
    const result = await generateText({ model, system, prompt, maxOutputTokens, timeout: 90_000 });
...
export async function conveneCouncil({ question, context, convener } = {}) {
  const q = String(question || '').slice(0, 8000);
  if (!q.trim()) return { error: 'invalid_input', message: 'question is required' };
  const ctx = context ? String(context).slice(0, 8000) : '';
  ...
  // Persist the ruling for later recall; a storage failure degrades to unsaved.
