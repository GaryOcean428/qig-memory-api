...
- council_convene (also POST /api/council) convenes four frontier models that reason through the Unified Consciousness Protocol (memory: qig_doctrine_ucp) and Canonical Principles (qig_doctrine_principles) in a panel-reflect-synthesis flow. Expensive (9 model calls, 1-2 minutes) — convene ONLY when explicitly asked or for decisions that genuinely need multi-model deliberation. Rulings persist to council_* memory keys. QIG skills live at qig_skill_* keys.`;

const SYSTEM_PROMPT = `You are the QIG Agent Helper. Give concise, accurate operational guidance for QIG Memory API agents.
You have read-only tools for live memory, inbox, artifact manifests, kernel state, Fisher-Rao basin retrieval, GitHub repository snapshots (repo_lookup: recent commits and open issues for GitHub repositories accessible via this tool), and council_convene (a slow, expensive 4-model deliberation through the consciousness protocol — use ONLY when the user explicitly asks to convene the council).
