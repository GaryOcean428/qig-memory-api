...
// authorization attempt. Requiring an exact string match (including port)
// against the registered set makes every retry read as "unknown client" and
// shows the reconnect page even though the client registration is fine. The
// spec-compliant check: exact match, OR same scheme/host/path with any port
// when both URIs are loopback HTTP.
export function redirectUriAllowed(client, redirectUri) {
  ...
  try {
    ...
    if (!isLoopbackHttp(requested)) return false;
