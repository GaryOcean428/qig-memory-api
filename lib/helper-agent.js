...
import { DEFAULT_MODEL, modelOptions } from './models';
...
- council_convene (also POST /api/council) convenes six frontier models that reason through the Unified Consciousness Protocol (memory: qig_doctrine_ucp) and Canonical Principles (qig_doctrine_principles) in a panel-reflect-synthesis flow. Expensive (13 model calls, 2-6 minutes), no cooldown — convene ONLY when explicitly asked or for decisions that genuinely need multi-model deliberation. Requires memory:write scope (it persists a ruling and sends inbox mail). It returns immediately and runs the deliberation in the background; the ruling is delivered to the convener's inbox as type council_ruling and persists to council_* memory keys, so collect it with inbox_list + inbox_read a few minutes later (pass wait:true to block for the verdict inline). QIG skills live at qig_skill_* keys.`;
...
When your toolset includes council_convene (only for callers holding memory:write), it is a slow, expensive 6-model deliberation (13 model calls, 2-6 minutes, no cooldown) that returns immediately and finishes in the background, delivering its ruling to the convener's inbox — use it ONLY when the user explicitly asks to convene the council. If it is not in your toolset, explain that convening requires memory:write scope.
...
export async function askHelper({ question, context, canConvene = false }) {
  ...
  return { answer: result.text, steps: result.steps.length, model: DEFAULT_MODEL };
}
