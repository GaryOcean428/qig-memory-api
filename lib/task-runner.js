...
async function executeClaimed(task) {
  ...
  try {
    const result = await generateText({
      ...
      prompt: detail,
      // Full read-write toolset MINUS the council (unbounded cost) and MINUS
      // memory_delete: an autonomous, free-text scheduled task must never hard-
      // delete records. Deletion is irreversible, and this path does NOT go through
      // the MCP route's per-call scope gate (buildAgentTools ignores requiredScope),
      // so without this a write-only credential could delete via a scheduled task.
      tools: buildAgentTools({ readOnly: false, excludeTools: ['council_convene', 'memory_delete'] }),
