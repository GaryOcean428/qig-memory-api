...
import { extractFailurePatterns } from './learn';
...
export async function runDailyReview({ trigger = 'cron' } = {}) {
  ...
  // Phase 5 — failure pattern extraction. Scans task_run_* records for failures
  // and persists learned_pattern records for future retrieval boosting.
  let failurePatterns = { extracted: 0, patterns: [] };
  try {
    failurePatterns = await extractFailurePatterns();
    phases.failure_extraction = { extracted: failurePatterns.extracted };
  } catch (error) {
    phases.failure_extraction = { error: error.message?.slice(0, 160) };
  }

  // Phase 6 — rebuild search index. Keeps the inverted index fresh so
  // term-based searches stay O(matches) instead of degrading to O(corpus).
  try {
    const { buildSearchIndex } = await import('./search-index');
    const indexMeta = await buildSearchIndex();
    phases.search_index = { rebuilt: true, ...indexMeta };
  } catch (error) {
    phases.search_index = { error: error.message?.slice(0, 160) };
  }
