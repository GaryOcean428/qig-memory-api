...
export async function setClientTrust(clientId, trusted) {
  const client = await getClient(clientId);
  if (!client) return null;
  const updated = {
    ...client,
    trusted: Boolean(trusted),
    approved_scopes: trusted
...
export async function getOAuthPrincipal(token) {
  ...
  const scopes = approved.filter((scope) => tokenScopes.has(scope) || tokenScopes.has('mcp:tools'));
