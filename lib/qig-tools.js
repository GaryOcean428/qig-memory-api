    ...
    execute: async (args) => {
      ...
      if (res.has_more || res.truncated) {
        res._pagination = {
          ...
          truncated: !!res.truncated,
          hint: res.truncated
            ? `Scan cap reached: results are newest-first and the RECENT window is complete, but older messages beyond the cap were not examined. Page with cursor="${res.cursor}" for older mail.`
