...
//
// Budget arithmetic (derived from the constants below at every reference —
// this is the value that rotted before: a comment restating "~5x150s fits"
// silently stopped being true the moment synthesis's fallback was added.
// Nothing here is a restated number; MEMBER_TIMEOUT_MS and
// CONVENE_MAX_DURATION_S are the only two inputs and the guard below reads
// them directly, so this text and the code it describes cannot drift apart):
//   - worst case per seat  = MEMBER_TIMEOUT_MS*2 (primary + fallback, both
//     exhausted) = 300s
//   - worst case per phase = ceil(6 / CONCURRENCY) * (MEMBER_TIMEOUT_MS*2)
//   - phases are SEQUENTIAL: panel + reflect + synthesis (synthesis is one
//     seat: MEMBER_TIMEOUT_MS..MEMBER_TIMEOUT_MS*2)
//   - route budget: CONVENE_MAX_DURATION_S (both app/api/council/route.js and
//     app/api/mcp/route.js declare `maxDuration = 800` as a literal — Next.js
//     requires a literal there, so keep it in sync with this constant by hand)
// At CONCURRENCY=6: 300 + 300 + 150..300 = 750..900s. THIS CAN EXCEED THE
// 800s ROUTE BUDGET — the old comment claiming "~5x150s fits" assumed
// synthesis never needs its fallback, which is not guaranteed. The fix is
// NOT tighter arithmetic (there is no seat count / concurrency / timeout
// combination at full width that both fits every fallback-heavy edge case
// AND keeps the 90s->150s fix for mid-retry aborts); it is the deadline-aware
// guard in callMember below, which makes the ACTUAL wall-clock, not this
// worst-case sum, the thing that decides whether a fallback attempt runs at
// all. At CONCURRENCY=3: ceil(6/3)=2 rounds per phase -> 600 + 600 + 300 =
// 1500s — nearly double the budget, relying on the same guard even harder.
// Lowering the knob below 6 shrinks the safety margin further; the guard is
// what makes any of these survivable, not this arithmetic.
//
...
// Per-attempt timeout used by callMember (primary AND fallback each get up to
// this much, subject to the deadline guard below). Extracted from the literal
// that used to live inline so the budget-arithmetic comments above can derive
// from it instead of restating "150s" as prose.
const MEMBER_TIMEOUT_MS = 150_000;

// The route's declared wall-clock budget. Investigated 2026-07-23: Vercel's
// GA ceiling for Pro/Enterprise with fluid compute is 800s (this project is
// necessarily on Pro or Enterprise, since Hobby caps at 300s and this route
// already runs at 800 in production) — see
// https://vercel.com/docs/functions/configuring-functions/duration. A higher
// "Extended max duration" tier exists (up to 1800s) but is an opt-in BETA
// gated on team enrollment and specific Node runtime pins, is incompatible
// with Secure Compute/Static IPs, and its availability for THIS team cannot be
// confirmed from within the repo (no dashboard/API access in this session) —
// so 800 is the only ceiling that can be relied on without risking an
// unverified deploy-time rejection. MUST match the literal `maxDuration`
// exports in app/api/council/route.js and app/api/mcp/route.js.
export const CONVENE_MAX_DURATION_S = 800;

// Reserved off the platform's hard kill for the post-synthesis bookkeeping
// (persist the report, then up to 3 inbox-delivery attempts with backoff —
// see the tail of conveneCouncil) that must complete for the "done isn't
// delivered" and F3-zombie guarantees elsewhere in this file to hold. Cheap
// blob/queue writes measured at ~0.3s each (ec484a1); 20s is generous
// headroom, not a tuned-to-the-second budget.
const CONVENE_SAFETY_MARGIN_MS = 20_000;

// The actual deadline every model call in a convene is measured against.
// Computed from the two constants above, not restated.
const CONVENE_BUDGET_MS = CONVENE_MAX_DURATION_S * 1000 - CONVENE_SAFETY_MARGIN_MS;

// Below this much remaining budget, do not even START an attempt (primary or
// fallback): reasoning:'high' plus tool round-trips need real time to
// produce anything, so a call given less than this is very likely to just
// fail anyway, and it is better spent letting the phase finish and moving on
// than burning the last of the budget on a near-certain failure. Not derived
// from another constant — it is a floor on what a model call can plausibly
// do, not a fraction of the route budget.
const MIN_MEMBER_ATTEMPT_MS = 30_000;
...
// Bounded concurrency for the reap/redeliver record loops below (fix/inbox-sweep-timeout,
// d663803, ec484a1 precedent: sequential per-record blob round trips on the SAME hourly
// cron route already timed out 22x in production for sweepInbox before it was
// parallelized). Each record here does 1-3 blob round trips (putMemory, or
// getMemory+sendInboxMessage+putMemory for redelivery) — much cheaper than a council
// seat's ~150s LLM call, so this pool can run wider than COUNCIL_CONCURRENCY (6) without
// the wall-clock risk that constant is tuned for. It is still BOUNDED, deliberately NOT
// a single Promise.all over up to 200 records at once: firing 200 concurrent writes at
// the blob store in one burst is its own failure mode (thundering-herd on the store),
// distinct from the wall-clock problem this fixes. 10 keeps a worst-case 200-record
// batch to ~20 sequential rounds (seconds, not the tens-of-seconds a fully sequential
// loop could reach) while capping the simultaneous fan-out; tune via
// QIG_COUNCIL_REAP_CONCURRENCY if the store's real limits need it.
const REAP_CONCURRENCY = Math.max(1, Number(process.env.QIG_COUNCIL_REAP_CONCURRENCY) || 10);
...
// `deadlineAt` is an absolute Date.now()-scale timestamp shared by every call in
// one convene (see CONVENE_BUDGET_MS / conveneCouncil). It is what makes the
// route's ACTUAL elapsed wall-clock, not the worst-case arithmetic in the
// comment above COUNCIL_CONCURRENCY, the thing deciding whether an attempt
// runs: each attempt's own timeout is shortened to whatever is actually left,
// and an attempt that would start with less than MIN_MEMBER_ATTEMPT_MS
// remaining is skipped outright rather than started and platform-killed.
// This is what lets a convene degrade to a partial-but-synthesizable panel
// instead of dying with nothing persisted when the 750..900s worst case
// exceeds the 800s route budget.
async function callMember(member, { system, prompt, maxOutputTokens, tools, deadlineAt }) {
  const remainingMs = () => (deadlineAt ? deadlineAt - Date.now() : Infinity);
  const attempt = async (model, timeoutMs) => {
    const result = await generateText({
      model,
      ...modelOptions(model),
      system,
      ...
      // Was a flat 150s (itself raised from 90s: reasoning:'high' + up to
      // MEMBER_TOOL_STEPS store round-trips + the gateway's ZDR fallback chain
      // must all fit in ONE attempt, and 90s aborted members mid-retry ("Delay
      // was aborted")). Now shortened to whatever the convene deadline leaves —
      // never more than MEMBER_TIMEOUT_MS, never less than MIN_MEMBER_ATTEMPT_MS
      // (the caller does not reach here otherwise). This is an input to the
      // convene-wide budget arithmetic documented above COUNCIL_CONCURRENCY —
      // re-check that math before changing it.
      timeout: timeoutMs,
    });
  ...
  const primaryBudget = Math.min(MEMBER_TIMEOUT_MS, remainingMs());
  if (primaryBudget < MIN_MEMBER_ATTEMPT_MS) {
    console.log(`[v0] council member ${member.name} SKIPPED — only ${Math.round(primaryBudget / 1000)}s left on the convene deadline, below the ${MIN_MEMBER_ATTEMPT_MS / 1000}s floor`);
    return { name: member.name, model: member.model, error: 'deadline_exhausted: skipped — insufficient convene budget remaining for a primary attempt' };
  }
  try {
    return await attempt(member.model, primaryBudget);
  } catch (err) {
    if (member.fallback) {
      const fallbackBudget = Math.min(MEMBER_TIMEOUT_MS, remainingMs());
      if (fallbackBudget < MIN_MEMBER_ATTEMPT_MS) {
        const detail = describeModelError(err);
        console.log(`[v0] council member ${member.name} primary ${member.model} failed (${detail}); fallback SKIPPED — only ${Math.round(fallbackBudget / 1000)}s left on the convene deadline`);
        return { name: member.name, model: member.model, error: `${detail} | fallback skipped: deadline_exhausted` };
      }
      console.log(`[v0] council member ${member.name} primary ${member.model} failed (${describeModelError(err)}); retrying on ${member.fallback} with ${Math.round(fallbackBudget / 1000)}s left`);
      try {
        return await attempt(member.fallback, fallbackBudget);
      } catch (fallbackErr) {
...
export async function reapStaleCouncilJobs({ nowMs = Date.now() } = {}) {
  ...
  try {
    ...
    // Records are independent (distinct keys, no shared state, no conditional/ifMatch
    // write) so each one's parse-check-maybe-write sequence can run concurrently.
    // Bounded via REAP_CONCURRENCY — see its definition for why 10 and why not
    // Promise.all over the whole batch.
    await mapWithConcurrency(
      ...
      async (rec) => {
        ...
        if (!Number.isFinite(startedMs) || nowMs - startedMs < RUNNING_ZOMBIE_MS) return;
...
export async function redeliverUndeliveredRulings({ limit = 200 } = {}) {
  ...
  try {
    ...
    // Same reasoning as reapStaleCouncilJobs: each record's marker-check-then-deliver
    // sequence touches only that record's own keys (`rec.key`, `${rec.key}__delivered`),
    // so records are independent of one another and safe to run concurrently. Within a
    // single record the read-then-write ordering (check marker, THEN send, THEN write
    // marker) is preserved — only the ACROSS-record loop is parallelized. Bounded via
    // REAP_CONCURRENCY.
    await mapWithConcurrency(
      results,
      async (rec) => {
        if (rec.key.endsWith('__delivered')) return; // skip our own delivery markers
        let parsed;
        try {
          parsed = JSON.parse(rec.content);
        } catch {
        ...
        if (parsed?.status !== 'completed' || !parsed.convener) return; // broadcast rulings need no targeted redelivery
        if (parsed.verdict == null && !parsed.synthesis_error) return; // nothing to deliver
        examined += 1;
        const marker = await getMemory(`${rec.key}__delivered`).catch(() => null);
        if (marker?.content) return; // already delivered
        try {
...
export async function conveneCouncil({ question, context, convener, convenedAt } = {}) {
  ...
  // Absolute deadline for every model call this convene makes (panel, reflect,
  // synthesis alike) — see CONVENE_BUDGET_MS / callMember. Anchored to when
  // THIS function started, not `convenedAt` (which can predate this call by
  // however long the tool's ACK phase took) — the wall-clock the platform
  // will actually kill is measured from function entry.
  const deadlineAt = Date.now() + CONVENE_BUDGET_MS;
...
tool call, or your seat is lost from the panel.`;
  ...
  // Phase 3 — SYNTHESIS. Also deadline-guarded: if panel+reflect ran long, the
  // synthesizer's own primary/fallback attempts are shortened or skipped the
  // same way a member's would be, rather than run unconstrained and risk the
  // platform killing the function mid-synthesis with the report not yet
  // persisted (the persist happens below, after this call).
