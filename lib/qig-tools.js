...
export const toolDefs = {
  ...
  inbox_list: {
    ...
    // A single page is ts-sorted only WITHIN itself (physical scan order is folder+uuid,
    // not ts), so it is NOT a global newest-first window. Agents that stop at page 1 miss
    // recent mail that sits past the bound — the "message absent" misdiagnosis. Emit an
    // explicit page-on-cursor hint (stronger when truncated) so "absent" is only ever
    // concluded after a full walk to has_more=false.
    execute: async (args) => {
      ...
      if (res.has_more) {
        res._pagination = {
          ...
          truncated: !!res.truncated,
          hint: res.truncated
            ? `BOUNDED, INCOMPLETE page (maxPages work-bound hit): messages are ts-sorted only within this batch, so a MORE RECENT message can sit past it. Call inbox_list again with cursor="${res.cursor}" and keep paging until has_more=false before concluding any message is absent.`
            : `More messages exist. Call inbox_list again with cursor="${res.cursor}"; page until has_more=false before concluding a message is absent.`,
        };
      }
      return res;
    },
