...
async function signedUrl(pathname, operation, validUntil, options = {}) {
  // privateBlobOptions supplies either the static private token or explicit
  // OIDC credentials (oidcToken + storeId), both accepted by issueSignedToken.
...
export async function finalizeArtifact({ name, version }) {
  if (!SAFE_NAME.test(name) || !SAFE_VERSION.test(version)) {
    const error = new Error('invalid artifact name or version');
...
export async function getArtifactRows({ name, version, start, end, origin }) {
  ...
  if (!Number.isInteger(startRow) || !Number.isInteger(endRow) || startRow < 0 || endRow <= startRow || endRow > manifest.row_count) {
    const error = new Error(`row range must satisfy 0 <= start < end <= ${manifest.row_count}`);
    error.code = 'invalid_input';
    throw error;
  }
...
export async function streamArtifactRows({ name, version, start, end }) {
  ...
  if (!result || (result.statusCode !== 200 && result.statusCode !== 206)) return null;
