...
// Positive marker cache with a TTL: ns -> recheck-after epoch ms. Markers
// normally only go absent -> present, but the TTL means DELETING the marker
// blobs is a working rollback lever — every warm instance re-checks within
// TTL and falls back to the full scan. (A permanent positive cache would pin
// warm instances to the index path until they were recycled.)
const TS_MARKER_RECHECK_MS = 5 * 60 * 1000;
const tsIndexReadyCache = new Map();

function cacheMarkerPositive(ns) {
  tsIndexReadyCache.set(ns, Date.now() + TS_MARKER_RECHECK_MS);
}

async function tsIndexMarkersPresent(namespaces) {
  const now = Date.now();
  const pending = namespaces.filter((ns) => (tsIndexReadyCache.get(ns) ?? 0) <= now);
  if (pending.length === 0) return true;
  const found = await Promise.all(
    pending.map(async (ns) => {
      const marker = await readPrivateJson(tsStatePath(ns)).catch(() => null);
      if (marker?.data?.complete) {
        cacheMarkerPositive(ns);
        return true;
      }
      tsIndexReadyCache.delete(ns);
      return false;
    }),
  );
...
// the full-scan path. On Vercel an env-var change takes effect on the NEXT
// deployment, not on running instances — for an instant no-deploy rollback,
// delete the inbox-ts-state/ marker blobs instead: every instance reverts
// within the marker cache TTL (5 min). Either way the index keeps being
// WRITTEN, so it stays complete and reads can be re-enabled at any time.
...
// Server-side silo enforcement. `allowedNamespaces` is the caller's credential
// restriction (see lib/api-keys.js normalizeNamespaces): null/undefined means
// unrestricted (legacy keys, the env bootstrap key, OAuth principals, and every
// SYSTEM-INTERNAL caller that invokes sendInboxMessage with no options at all —
// council.js, task-runner.js, artifact-store.js, daily-reviewer.js). An array
// means the credential is restricted to exactly those namespaces; anything else
// is rejected server-side, closing the gap where enum validation alone let a
// key scoped to one silo write another's namespace.
//
// FAIL CLOSED, not just fail-array-checked: only a literal null/undefined
// skips the check. Anything else that isn't an array (a malformed caller
// passing a string, an object, NaN, ...) must DENY rather than silently fall
// through the `Array.isArray` gate as "unrestricted" — a bug that hands a
// caller a bad value must never be equivalent to granting it full access.
export async function sendInboxMessage(input, { allowedNamespaces } = {}) {
  const parsed = inboxSendSchema.parse(input);
  if (allowedNamespaces != null && (!Array.isArray(allowedNamespaces) || !allowedNamespaces.includes(parsed.namespace))) {
    const error = new Error(
      `key not permitted to write namespace "${parsed.namespace}" (allowed: ${Array.isArray(allowedNamespaces) ? allowedNamespaces.join(', ') || 'none' : 'malformed restriction'})`,
    );
...
// not blob reads) examined per call, counting ONLY entries at-or-past the
// cursor position. Cursor-skips are deliberately NOT counted: there is no
// seek — every call re-lists the prefix from the top and skips forward — so
// counting skips would let a resume cursor at depth >= cap burn its whole
// budget re-skipping and return the SAME cursor forever (a livelock, the
// filtered-pagination failure class this repo has hit three times, proven by
// the PR #79 red-team). With skips uncounted, every truncated call makes
// cap-worth of FRESH progress and its resume cursor strictly advances, so a
// deep drain always terminates. Recipient- and status-rejected entries past
// the cursor DO count — they are the work being bounded.
...
async function listViaTsIndex({ namespaces, wantRecipient, include_broadcast, status, safeLimit, cursorObj, now }) {
  // A hand-built cursor with an unparseable ts must behave like NO cursor
  // (serve page 1), matching the legacy path's effective behavior — not
  // reject every entry and report an empty inbox.
  if (cursorObj && !Number.isFinite(Date.parse(cursorObj.ts))) cursorObj = null;
  ...
  while (!truncated && matches.length <= safeLimit) {
    ...
    while (batch.length < wanted && !truncated) {
      ...
      const entry = best.buffer[0];
      // Cursor: everything STRICTLY older than the (ts,id) position. Entries
      // at-or-before the cursor are SKIPS — consumed but NOT counted against
      // the cap (see the cap comment: counting them livelocks deep resumes)
      // and never recorded as lastExamined (the resume position must not
      // regress to before the cursor).
      if (cursorObj) {
        const older = entry.ms < cursorMs
          || (entry.ms === cursorMs && String(entry.id) < String(cursorObj.id));
        if (!older) { best.buffer.shift(); continue; }
      }
      // Cap check BEFORE consuming a COUNTABLE entry: every entry that counts
      // as examined must be fully evaluated, or the resume cursor
      // (strictly-older-than lastExamined) would skip the entry that tripped
      // the cap forever.
...
export async function sweepInbox({ limit = 250, cursor, reindex = false } = {}) {
  ...
  if (walkClean && !page.hasMore) {
    try {
      ...
      INBOX_NAMESPACES.forEach((ns) => cacheMarkerPositive(ns));
...
// Cross-invocation walk state for the DAILY cron (PR #81 moved it from
// hourly). Persisted OUTSIDE the per-namespace marker names.
const SWEEP_WALK_STATE_PATH = `${TS_STATE_PREFIX}walk.json`;
// A persisted walk older than this is abandoned — start fresh rather than
// resume a stale snapshot of the namespace.
const SWEEP_WALK_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;

// Drive a sweep CHAIN to completion within one invocation's page budget,
// resuming a persisted walk across invocations when the namespace is too big
// for one budget.
//
// Budget arithmetic (per 1000-blob page, worst case during migration):
// 1 list + one full-page parallel read wave (~1-2s) + up to 1000 backfill
// writes at 32-wide (~10s at ~0.3s/write) ≈ ~12s. The default pageBudget of
// 20 is therefore ~240s worst case — inside the route's 300s maxDuration —
// and covers a ≤20,000-message inbox in a single daily invocation.
// Post-migration pages skip backfill (~2-3s each), so 20 pages ≈ ~60s.
//
// Beyond ~20k messages a single invocation cannot finish the walk, so an
// UNFINISHED but still-clean chain persists its composite cursor (which
// carries the started-at-the-beginning + zero-failures + zero-deletions
// proof) and the next invocation RESUMES it; minting therefore survives
// multi-invocation walks. Sends during the gap self-index, so the proof
// still holds across the gap. The gap does widen the documented residual
// hazard — another caller's sweep deleting expired mail mid-walk is
// undetectable by this chain's own counters — from minutes to days;
// inbox_sweep {reindex:true} remains the repair. A chain that gets poisoned
// (failure or deletion) or that finishes clears the persisted state; state
// is only ever persisted while the markers are still absent (a post-marker
// repair pass past the budget is paged manually via sweepInbox cursors).
export async function sweepInboxChain({ pageBudget = 20, limit = 1000, reindex = false } = {}) {
  const chain = {
    pages: 0,
    scanned: 0,
    deleted: 0,
  ...
  let cursor;
  const markersAlready = await tsIndexMarkersPresent(INBOX_NAMESPACES).catch(() => false);
  if (!markersAlready) {
    const saved = await readPrivateJson(SWEEP_WALK_STATE_PATH).catch(() => null);
    const savedAt = saved?.data?.saved_at ? Date.parse(saved.data.saved_at) : NaN;
    if (saved?.data?.cursor && Number.isFinite(savedAt) && Date.now() - savedAt <= SWEEP_WALK_MAX_AGE_MS) {
      cursor = saved.data.cursor;
      chain.resumed = true;
    }
  }
  for (let i = 0; i < Math.max(1, pageBudget); i += 1) {
    const res = await sweepInbox({ limit, cursor, reindex });
    chain.pages += 1;
    chain.scanned += res.scanned;
    chain.deleted += res.deleted;
    chain.indexed += res.indexed;
    chain.failures.push(...res.failures);
    chain.reindex_complete = chain.reindex_complete || !!res.reindex_complete;
    chain.has_more = !!res.has_more;
    if (!res.has_more) { cursor = null; break; }
    cursor = res.cursor;
  }
  if (!chain.has_more) {
    await deletePrivate(SWEEP_WALK_STATE_PATH).catch(() => {});
    return chain;
  }
  const { full } = decodeSweepCursor(cursor);
  const stillNeeded = !(await tsIndexMarkersPresent(INBOX_NAMESPACES).catch(() => false));
  if (full && stillNeeded) {
    // Unfinished but clean, and the markers still need minting: hand the walk
    // to the next invocation.
    await writePrivateJson(
      SWEEP_WALK_STATE_PATH,
      { cursor, saved_at: new Date().toISOString() },
      { allowOverwrite: true },
    ).catch(() => {});
