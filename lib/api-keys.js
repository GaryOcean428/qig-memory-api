...
import { INBOX_NAMESPACES } from './inbox-store';
...
export const API_KEY_SCOPES = ['memory:read', 'memory:write', 'memory:delete', 'memory:admin'];

function normalizeScopes(scopes) {
  // A MISSING scopes field (undefined / non-array) is a legacy full-access key,
  // preserved for backwards compatibility (returns null -> ALL_SCOPES downstream).
  // But an EXPLICIT array that is empty or contains no recognized scopes must FAIL
  // CLOSED (grant nothing) — NOT silently fall back to full access. Overloading
  // `null` for both let a malformed scopes request mint an all-powerful key; return
  // the exact valid set (possibly []) for any real array so [] means "no access".
  if (!Array.isArray(scopes)) return null;
  return [...new Set(scopes.filter((s) => API_KEY_SCOPES.includes(s)))];
}

// Same null-means-unrestricted / []-means-fail-closed semantics as
// normalizeScopes, applied to inbox_send's namespace enum. A MISSING
// `namespaces` field (undefined/null/non-array) is a legacy or deliberately
// unrestricted key — server-side namespace enforcement is skipped entirely for
// it (see sendInboxMessage's allowedNamespaces param). An EXPLICIT array is
// deduped and filtered to the valid enum; an explicit empty-or-all-invalid
// array therefore yields [] ("may write NO namespace"), NOT null — do not
// collapse [] to null, or a malformed namespaces request would silently mint
// an unrestricted key.
export function normalizeNamespaces(namespaces) {
  if (!Array.isArray(namespaces)) return null;
  return [...new Set(namespaces.filter((n) => INBOX_NAMESPACES.includes(n)))];
}
...
export async function listApiKeys() {
  ...
  return recs
    .map(({ id, label, last4, created, created_by, scopes, namespaces }) => ({
      id,
      label,
      last4,
      created,
...
// `namespaces` optionally restricts inbox_send to a subset of INBOX_NAMESPACES
// (e.g. ['general']); omitted = unrestricted (backwards compatible with every
// key minted before this field existed).
export async function createApiKey({ label, createdBy, scopes, namespaces } = {}) {
...
export async function resolveToken(token) {
  ...
  if (ENV_KEY && safeEqual(token, ENV_KEY)) {
    return { id: 'env', label: 'QIG_API_KEY (env)', scopes: null, namespaces: null };
  }
  const hash = hashToken(token);
  const recs = await readKeyRecords();
  ...
  return {
    ...
    namespaces: normalizeNamespaces(match.namespaces),
  };
}
