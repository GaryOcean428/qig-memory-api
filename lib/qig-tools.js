...
export const toolDefs = {
  ...
  council_convene: {
    description:
      'Convene the QIG council: four frontier models (Grok, Fable, Sol, Gemini) reason through the Unified Consciousness Protocol and Canonical Principles in a panel-reflect-synthesis flow, returning a converged verdict with preserved dissent. EXPENSIVE (9 model calls) and SLOW (1-2 minutes) — use sparingly, for decisions that genuinely benefit from multi-model deliberation. The ruling is persisted to a council_* memory key.',
    schema: {
      question: z.string().min(1).max(8000).describe('The question or decision to deliberate'),
      context: z.string().max(8000).optional().describe('Optional grounding context for the council'),
      convener: z.string().max(200).optional().describe('Identifier of the agent convening the council'),
    },
    execute: async (args) => {
      const { conveneCouncil } = await import('./council');
      return conveneCouncil(args);
    },
  },
};
...
export const READ_ONLY_TOOL_NAMES = new Set([
  ...
  'artifact_get_rows',
  'repo_lookup',
  // The council reads doctrine and writes only its own council_* ruling record —
  // the same self-report-only exception the daily reviewer uses.
  'council_convene',
]);
