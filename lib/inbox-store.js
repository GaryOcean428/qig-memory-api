...
export async function listInboxMessages({
  ...
  const { results, cursor: nextCursor, has_more, truncated } = await collectFilteredPage({
    ...
    cursor,
    maxPages: 8,
    label: `inbox:ns=${namespace || '*'},to=${wantRecipient || '*'},status=${status || '*'}`,
  });
  // Physical scan order is folder+uuid, NOT ts, so this batch is ts-sorted only WITHIN
  // itself — a bounded page is not a global newest-first window. `truncated` warns the
  // caller the page is incomplete (page on has_more); a recent message can sit past it.
