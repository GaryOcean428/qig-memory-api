...
import { normalizeNamespaces } from './api-keys';
...
// 12h (was 1h). The refresh grant below is now loss-tolerant, but a 1h access
// TTL still forced the refresh round-trip hourly — every one a chance to hit a
// dropped response. A longer access life cuts that exposure ~12x.
const ACCESS_TTL_SECONDS = 12 * 60 * 60;
const CODE_TTL_SECONDS = 5 * 60;
const REFRESH_TTL_MS = 30 * 24 * 60 * 60 * 1000;
// Idempotent-replay window for refresh rotation: if the server rotates but the
// response never reaches the client (network drop, client killed before it
// persists the new token), a replay of the SAME old refresh token within this
// window returns the SAME already-issued pair instead of invalid_grant. It never
// mints a second pair, so a retry storm cannot proliferate live tokens.
const REFRESH_GRACE_MS = 90 * 1000;
const ROTATED_PREFIX = `${PREFIX}rotated/`;
...
// Stable identity for a public client: the callback URL set plus the declared
// name. Two DCR calls with the same fingerprint describe the same logical
// client, so we can safely return the original record instead of minting a new
// one (RFC 7591 permits reuse; PKCE still protects each exchange).
// Loopback URIs are normalized WITHOUT their port: native clients (and
// proxying connectors such as Headroom) bind an ephemeral port per attempt
// (RFC 8252 §7.3), so the port carries no client identity. Without this, every
// OAuth retry minted a brand-new client record — proliferating dead clients
// and burning the per-IP registration rate limit.
function normalizeRedirectUriForFingerprint(value) {
  // Loopback identity = path only (host form is rewritten in transit and the
  // port is ephemeral) — one logical client regardless of which form/port a
  // given attempt used. loopbackPath is defined below with the redirect
  // matchers; function declarations hoist, so this is safe.
  const path = loopbackPath(value);
  return path !== null ? `loopback:${path}` : String(value);
}

function clientFingerprint(redirectUris, clientName) {
  const canonical = JSON.stringify({
    redirect_uris: [...redirectUris].map(normalizeRedirectUriForFingerprint).sort(),
...
// Native MCP hosts (Cursor, VS Code, Windsurf, …) register private-use
// redirect schemes (RFC 8252 §7.1). Allow ONLY known client schemes so
// Hermes/Claude/loopback HTTPS paths stay unchanged and random
// evilapp://… registrations are rejected. Require a host label.
const ALLOWED_PRIVATE_USE_REDIRECT_SCHEMES = new Set([
  'cursor:',
  'vscode:',
  'vscode-insiders:',
  'windsurf:',
...
function isAllowedPrivateUseRedirectUri(url) {
  if (!ALLOWED_PRIVATE_USE_REDIRECT_SCHEMES.has(url.protocol)) return false;
  // Native app callbacks need a host (reverse-DNS style).
...
export function isSafeRedirectUri(value) {
  try {
    ...
    // Unchanged for non-Cursor agents: HTTPS anywhere, HTTP loopback only.
    if (url.protocol === 'https:') return true;
    if (url.protocol === 'http:') {
      return ['localhost', '127.0.0.1', '[::1]'].includes(url.hostname);
    }
    return isAllowedPrivateUseRedirectUri(url);
  } catch {
    return false;
  }
}
...
// The identity of a loopback redirect URI is its PATH — nothing else. Native
// clients bind an ephemeral port per attempt (RFC 8252 §7.3), and the host
// FORM is unstable in transit: Vercel's edge rewrites 127.0.0.1 → localhost
// inside query strings (observed 2026-07-21: DCR JSON bodies persist
// 127.0.0.1 while authorize query params arrive as localhost, so exact-match
// validation failed for every CLI that registered the 127.0.0.1 form).
// Returns the pathname for loopback HTTP URIs, null otherwise.
function loopbackPath(value) {
  try {
    const url = new URL(String(value));
    return isLoopbackHttp(url) ? url.pathname : null;
  } catch {
    return null;
  }
}

// Two redirect URIs are equivalent when identical, or when both are loopback
// HTTP with the same path (any port, any loopback host form). HTTPS URIs are
// never treated as equivalent — exact match only.
export function redirectUrisEquivalent(a, b) {
  if (String(a) === String(b)) return true;
  const pa = loopbackPath(a);
  return pa !== null && pa === loopbackPath(b);
}

// RFC 8252 §7.3: for loopback redirect URIs the port is ephemeral — native
// apps (Hermes, grok CLI, Claude Code, mcp-remote) bind a RANDOM free port per
// authorization attempt, and proxies/edge layers may rewrite the loopback
// host form. Exact string matching makes every retry read as "unknown
// client". Match by equivalence instead.
export function redirectUriAllowed(client, redirectUri) {
  const registered = client?.redirect_uris || [];
  return registered.some((uri) => redirectUrisEquivalent(uri, redirectUri));
}
...
export async function consumeConsentToken(token, expected) {
  ...
  return Boolean(
    ...
    // Loopback redirect forms drift between the GET (query string, edge-
    // rewritten) and the consent POST (form body, not rewritten) — compare
    // by equivalence, not byte equality.
    redirectUrisEquivalent(record.redirect_uri, expected.redirectUri) &&
...
export async function consumeAuthorizationCode({ code, clientId, redirectUri, codeVerifier }) {
  ...
  // redirect_uri drifts host-form between authorize (query, edge-rewritten to
  // localhost) and token exchange (POST body, raw 127.0.0.1) — equivalence,
  // not byte equality, or every loopback code exchange fails as invalid_grant.
  if (!safeEqual(record.client_id, clientId) || !redirectUrisEquivalent(record.redirect_uri, redirectUri)) {
    return null;
  }
...
export async function refreshTokens({ refreshToken, clientId }) {
  ...
  const rotatedPath = `${ROTATED_PREFIX}${hash(refreshToken)}.json`;

  // First genuine use of this refresh token wins the single-use claim.
  if (await claimOnce('refresh', refreshToken)) {
    ...
    const issued = await issueTokens({ clientId: record.client_id, userId: record.user_id, scope: record.scope });
    if (issued) {
      // Stash the issued pair so a lost-response replay returns the SAME pair.
      // Gated by expires_at (below), so a lingering blob is dead after the window.
      await writeJson(rotatedPath, { client_id: record.client_id, issued, expires_at: Date.now() + REFRESH_GRACE_MS });
    }
    return issued;
  }

  // Already claimed — the client may simply never have received the first
  // response. Replay the SAME issued pair if we're still inside the grace window.
  const rotated = await readJson(rotatedPath);
  if (rotated && typeof rotated.expires_at === 'number' && rotated.expires_at >= Date.now()
      && safeEqual(rotated.client_id, clientId)) {
...
export async function getOAuthPrincipal(token) {
  ...
  return {
    ...
    // OAuth clients have no `namespaces` field today -> always null
    // (unrestricted). Intentional: OAuth is the operator/agent identity path,
    // not the credential-minting surface namespace restriction targets.
    // Normalized (not a raw `?? null`) so a malformed/foreign value on a
    // client record can never fail OPEN — normalizeNamespaces returns null
    // only for a true non-array, and [] for anything else invalid, matching
    // the fail-closed contract sendInboxMessage now enforces.
    namespaces: normalizeNamespaces(client.namespaces),
  };
}
