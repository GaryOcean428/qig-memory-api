...
export async function sweepInbox({ limit = 250, cursor } = {}) {
  ...
  let deleted = 0;
  const failures = [];
  // Read the page's blobs CONCURRENTLY, not in a sequential for-loop. Each private-blob
  // read is a network round trip (~0.3s); a sequential scan of a full page (up to 1000,
  // as the hourly cron requests) took long enough to blow the route's maxDuration
  // (Vercel Runtime Timeout Error, /api/cron/inbox-sweep, 60s, count=22 since 2026-03-15)
  // — the exact class of bug ec484a1 already fixed in listInboxMessages via Promise.all
  // (~30s -> ~1-2s for a 100-blob page), just not carried over to this function.
  ...
  for (const { blob, item, error } of reads) {
    ...
    try {
      ...
      failures.push({ pathname: blob.pathname, error: deleteError?.message || String(deleteError) });
    }
  }
  return {
