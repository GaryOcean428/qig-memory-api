...
async function mutateTask(id, mutator) {
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    try {
      const etag = await canonicalEtag(path(id));
      if (!etag) throw new Error(`cannot read etag for task ${id}`);
      await writePrivateJson(path(id), next, { allowOverwrite: true, ifMatch: etag });
      return next;
    } catch (error) {
...
export async function updateTask(id, patch) {
  ...
  return mutateTask(id, (task) => {
    ...
    if (clean.schedule) {
      ...
      // A one-off task that already ran has occurrenceCount >= 1, which makes
      // computeNextRun return null. Reset so the new schedule actually fires.
      if (next.schedule.kind === 'once' && next.occurrenceCount >= 1) next.occurrenceCount = 0;
    ...
    if (clean.status === 'scheduled' && !clean.schedule) {
      // A one-off task has already exhausted its single occurrence, so
      // computeNextRun would return null and leave it stuck. Reset the count
      // so re-activation actually schedules another run.
      if (next.schedule?.kind === 'once') next.occurrenceCount = 0;
      next.nextRunAt = computeNextRun(next.schedule, { occurrenceCount: next.occurrenceCount });
    }
