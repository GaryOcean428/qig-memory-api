...
function describeModelError(err) {
  ...
  for (let e = err, depth = 0; e && depth < 4; e = e.cause, depth += 1) {
    ...
    if (body) parts.push(`body=${String(typeof body === 'string' ? body : JSON.stringify(body)).slice(0, 300)}`);
    if (e.name && e.name !== err.name) parts.push(`cause=${e.name}`);
  }
  return [...new Set(parts)].join(' | ');
}

// Run tasks with BOUNDED concurrency. The panel/reflect phases used to fire all
// six seats at once, and each seat fans out to MEMBER_TOOL_STEPS more gateway
// round-trips — a burst big enough to trip a per-project concurrency/rate limit.
// On 2026-07-22 a single call to the very same model succeeded while the 6-wide
// convene lost 5 seats, which is the signature of a burst limit, not an outage.
// Sequential would waste the parallel budget; a small pool keeps the wall-clock
// while staying under the limit. COUNCIL_CONCURRENCY tunes it without a redeploy.
const COUNCIL_CONCURRENCY = Math.max(1, Number(process.env.QIG_COUNCIL_CONCURRENCY) || 3);
...
// Pre-convene health ping: ONE cheap call down the EXACT gateway path a seat uses
// (same modelOptions, same ZDR filter, same auth), so a convene can be checked
// before it spends instead of discovering a dead gateway 13 calls deep. On
// 2026-07-22 a single call to the very model five seats died on succeeded — so a
// one-call probe is a genuine discriminator, and it is ~1/13th of a convene's cost.
// Deliberately tiny: no tools, no reasoning, a handful of tokens, short timeout.
export async function councilHealthPing() {
  ...
  try {
    ...
    return { ok: true, model, ms: Date.now() - started, reply: (result.text || '').trim().slice(0, 32) };
  } catch (err) {
