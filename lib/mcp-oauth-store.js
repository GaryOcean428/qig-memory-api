...
// RFC 8252 §7.1 private-use URI schemes (Cursor/VS Code MCP OAuth use
// cursor://… / vscode://…). Block dangerous schemes; require a host label.
...
function isPrivateUseRedirectUri(url) {
  if (BLOCKED_REDIRECT_SCHEMES.has(url.protocol)) return false;
  // Must look like a real URI scheme (alpha then alpha/digit/+/-/.)
  if (!/^[a-z][a-z0-9+.-]*:$/i.test(url.protocol)) return false;
  // Native app callbacks need a host (reverse-DNS style) so we don't accept
  // opaque scheme-only strings.
...
export function isSafeRedirectUri(value) {
  try {
    ...
    return isPrivateUseRedirectUri(url);
  } catch {
    return false;
  }
}
