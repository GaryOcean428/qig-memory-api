  ...
  council_convene: {
    ...
    execute: async (args) => {
      ...
      return {
        ok: true,
        status: 'convening',
        note: `The council is convening now — 6 models across panel, reflect, and synthesis (2-6 minutes). You do NOT need to hold this call open. The ruling will be delivered to the "${to}" inbox in the qig namespace (type "council_ruling") and the full transcript to a council_* memory key. Check back in a few minutes with inbox_list + inbox_read, then inbox_ack once considered. Pass wait:true if you would rather block for the verdict inline.`,
        delivered_to: to,
      };
    },
  },
