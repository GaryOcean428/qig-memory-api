...
// useless for its actual purpose. Deletion (memory:admin) stays behind explicit
// operator approval, because losing memory is the only irreversible action here.
export const DEFAULT_CLIENT_SCOPES = ['memory:read', 'memory:write'];
export const OPERATOR_CLIENT_SCOPES = ['memory:read', 'memory:write', 'memory:admin'];
