    ...
    execute: async (args) => {
      ...
      if (res.has_more || res.truncated) {
        res._pagination = {
          ...
          truncated: !!res.truncated,
          hint: res.truncated
            ? `Namespace exceeds the scan cap: results are newest-first and the RECENT window is complete, but the OLDEST messages beyond the cap are not shown (the ts-ordered index is overdue). Page with cursor="${res.cursor}" for older mail.`
