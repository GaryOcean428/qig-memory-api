...
export const toolDefs = {
  ...
  council_health: {
    ...
    description:
      'CHEAP pre-convene health check: ONE small model call down the exact AI Gateway path a council seat uses (same routing, ZDR filter and auth), ~1/13th the cost of a convene. Returns {ok, model, ms} or {ok:false, error} with the REAL gateway status/body. Call this BEFORE council_convene to price-before-spend — a convene that burns 13 calls into a dead gateway is the failure this exists to prevent (2026-07-22: 5 of 6 seats lost, while a single call to the same model succeeded).',
    readOnlyHint: true,
    schema: {},
    execute: async () => {
      ...
      return {
        ...res,
        verdict: res.ok ? 'GREEN — gateway answered; a convene is worth the spend.' : 'RED — do NOT convene; the spend would burn into this fault.',
      };
    },
  },
