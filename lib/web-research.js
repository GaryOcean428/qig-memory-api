...
async function tavilyPost(path, body) {
  const key = tavilyKey();
  if (!key) {
    return { error: 'not_configured', message: 'Web research is unavailable: TAVILY_API_KEY is not set on this deployment.' };
  }
  let res;
  try {
  ...
  if (!res.ok) {
    ...
    const code = res.status === 401 || res.status === 403 ? 'auth_failed' : res.status === 429 ? 'rate_limited' : 'upstream_error';
