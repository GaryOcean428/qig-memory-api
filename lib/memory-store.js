...
const MEMORY_TOKEN = process.env.BLOB_READ_WRITE_TOKEN_2 || process.env.MEMORY_BLOB_READ_WRITE_TOKEN || '';

function memoryOptions(options = {}) {
  if (!MEMORY_TOKEN) throw new Error('A private Blob binding is required (BLOB_READ_WRITE_TOKEN_2 or MEMORY_BLOB_READ_WRITE_TOKEN)');
  return { ...options, token: MEMORY_TOKEN };
}
