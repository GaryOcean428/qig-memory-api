...
export async function sendInboxMessage(input, { allowedNamespaces } = {}) {
  ...
  if (Array.isArray(allowedNamespaces) && !allowedNamespaces.includes(parsed.namespace)) {
    const error = new Error(
      `key not permitted to write namespace "${parsed.namespace}" (allowed: ${allowedNamespaces.join(', ') || 'none'})`,
    );
