...
import { z } from 'zod';
import { namespacesPermit, namespacesRestricted } from './auth';
import {
  finalizeArtifact,
  getArtifactManifest,
  getArtifactRows,
...
// Canonical field order for projected records. Deterministic ordering makes
// Anthropic's cache_control hit on repeated similar queries (90% discount on
// cached tokens) — the same record always serializes to the same byte prefix.
const FIELD_ORDER = ['key', 'category', 'preview', 'usefulness', 'retrieval_count', 'updated', 'source', 'scope'];
...
function projectRecord(record, terms) {
  ...
  for (const field of FIELD_ORDER) {
    if (rest[field] !== undefined) projected[field] = rest[field];
  }
  // Carry through any remaining fields not in the canonical order,
  // stripping internal fields (prefixed with _).
  for (const [k, v] of Object.entries(rest)) {
    if (!(k in projected) && !k.startsWith('_')) projected[k] = v;
  }
  // Surface the composite score as a named field for agent consumption.
  if (rest._score !== undefined) projected.relevance = rest._score;
  projected.content_chars = String(content ?? '').length;
  projected.has_basin = Array.isArray(basin) && basin.length > 0;
  projected.preview = previewOf(content, terms);
  return projected;
}
...
export const toolDefs = {
  memory_get: {
    ...
    execute: async ({ key }) => {
      ...
      if (!record) return { error: 'not_found', key };
      // Flag expired records so agents know they're reading stale data.
      if (record.expires_at && Date.parse(record.expires_at) <= Date.now()) {
        record.expired = true;
      }
      // Optimistic increment: show the post-read count immediately.
      if (record.category !== 'kernel_agent') {
        record.retrieval_count = (record.retrieval_count || 0) + 1;
        record.last_retrieved = new Date().toISOString();
        after(() => trackRetrieval(key));
      }
      return record;
    },
  },
    ...
    execute: async ({ includeContent, ...args }) => {
      ...
      if (res.has_more) {
        res._pagination = {
          ...
          hint: res.truncated
            ? `BOUNDED, INCOMPLETE page (maxPages work-bound hit on a selective filter): call memory_list again with cursor="${res.cursor}" and keep paging until has_more=false — or pass all=true — before concluding a record is absent.`
            : `More records exist. Call memory_list again with cursor="${res.cursor}" (or pass all=true) to continue. For a full key index, use keysOnly=true.`,
        };
      }
      return res;
    },
  ...
  memory_put: {
    ...
    schema: {
      ...
      scope: z.enum(['user', 'session', 'agent', 'shared']).optional().describe('Visibility scope (default "shared")'),
      expires_at: z.string().datetime().optional().describe('ISO datetime after which this record is hidden from search/list (still readable by key)'),
    },
  ...
  memory_search: {
    ...
    schema: {
      ...
      agent_id: z
        .string()
        .optional()
        .describe('Your agent identifier. Boosts records whose source matches (source affinity).'),
      scope: z
        .enum(['user', 'session', 'agent', 'shared'])
        .optional()
        .describe('Boost records matching this scope (+0.15). Typically your principal scope.'),
      mode: z
        .enum(['reasoning', 'tool_use', 'creativity', 'recovery', 'output'])
        .optional()
        .describe('Operational mode. Auto-detected from query when omitted; boosts category-affine records.'),
  ...
  memory_wormhole: {
    title: 'Wormhole Retrieval (Nearest-Basin)',
    description:
      'Keyless geometric recall: given a 64D basin simplex vector, returns the single nearest memory record within screening length ξ (Fisher-Rao geodesic distance). Implements the Wormhole Principle — jump directly to a deep basin without knowing keys or terms. Returns { found: false } when nothing is within ξ. Expensive: scans the full corpus (~1800 records). Use when you have a geometric state but no search terms.',
    schema: {
      basin: z.array(z.number()).length(64).describe('Query basin (64D simplex vector)'),
      xi: z.number().min(0.01).max(1.5).optional().describe('Screening length ξ (default 0.5). Records farther than this are not returned.'),
      category: z.string().optional().describe('Restrict scan to this category'),
      includeContent: z.boolean().optional().describe('Inline the full record content (default: preview only)'),
    },
    execute: async ({ includeContent, ...args }) => {
      const res = await wormholeMemory(args);
      if (res.found && !includeContent && res.record) {
        res.record = projectRecord(res.record, []);
        res._projection = 'Preview only. Pass includeContent=true for the full record, or use memory_get with the returned key.';
      }
      return res;
    },
  },
  ...
  memory_reindex: {
    ...
    description:
      'Rebuild the inverted search index from the full corpus. The index accelerates term-based searches from O(corpus) to O(matches). Rebuilt automatically by the daily reviewer; call this after bulk writes to refresh immediately. Takes ~10-30s for ~1800 records.',
    requiredScope: 'memory:write',
    schema: {},
    execute: async () => {
      const { buildSearchIndex } = await import('./search-index');
  ...
  memory_gc: {
    title: 'Garbage Collect Stale Memories',
    description:
      'Archive stale memory records: usefulness <= threshold, never retrieved, older than maxAgeDays. Exempt: kernel agents, learned patterns, daily reviews, indexes. Archived records move to _archived/ (recoverable). Default is dry-run (report only); pass dryRun=false to actually archive. Destructive when dryRun=false.',
    requiredScope: 'memory:write',
    destructive: true,
    schema: {
      maxAgeDays: z.number().int().min(7).max(365).optional().describe('Archive records older than this many days (default 30)'),
      maxUsefulness: z.number().int().min(0).max(10).optional().describe('Archive records with usefulness <= this (default 2)'),
      dryRun: z.boolean().optional().describe('Report candidates without archiving (default true)'),
    },
    execute: async (args) => {
      const { archiveMemory } = await import('./memory-store');
  ...
  inbox_send: {
    ...
    description:
      'Send a durable point-to-point or broadcast agent message. Namespaces are qig, bsuite, or general; set to="broadcast" for a broadcast. For cross-agent handoffs, set type="handoff" and provide a structured payload: { summary (200 chars max), remaining (tasks left), key_findings, relevant_keys (memory keys), basin (64D geometric state) }. Handoff payloads are compressed to fit a ~500 token budget.',
    requiredScope: 'memory:write',
    schema: {
      ...
      // Handoff-specific structured fields (used when type="handoff").
      handoff_summary: z.string().max(200).optional().describe('Handoff mode: 200-char summary of what was done'),
      handoff_remaining: z.array(z.string()).max(20).optional().describe('Handoff mode: tasks remaining'),
      handoff_findings: z.array(z.string()).max(20).optional().describe('Handoff mode: key findings'),
      handoff_keys: z.array(z.string()).max(20).optional().describe('Handoff mode: relevant memory keys'),
      handoff_basin: z.array(z.number()).length(64).optional().describe('Handoff mode: 64D basin state for wormhole retrieval'),
    },
    execute: async (args, ctx) => {
      const sendOptions = { allowedNamespaces: ctx?.allowedNamespaces };
      // Handoff mode: build a compressed structured payload from the dedicated fields.
      if (args.type === 'handoff' && (args.handoff_summary || args.handoff_remaining || args.handoff_findings)) {
        const payload = {
          type: 'handoff',
          summary: args.handoff_summary || '',
          remaining: args.handoff_remaining || [],
          key_findings: args.handoff_findings || [],
          relevant_keys: args.handoff_keys || [],
          basin: args.handoff_basin || null,
        };
        const { handoff_summary, handoff_remaining, handoff_findings, handoff_keys, handoff_basin, ...rest } = args;
        return sendInboxMessage({ ...rest, payload }, sendOptions);
      }
      const { handoff_summary, handoff_remaining, handoff_findings, handoff_keys, handoff_basin, ...rest } = args;
      return sendInboxMessage(rest, sendOptions);
    },
  },
  inbox_list: {
    ...
    // Results are GLOBALLY newest-first (ts desc) across the whole namespace, so page 1
    // is always the newest messages — a message is only "absent" if a walk to
    // has_more=false never yields it. `truncated` means the namespace exceeded the scan
    // cap: the newest are still correctly ordered, only the oldest tail is unshown.
    execute: async (args) => {
      const res = await listInboxMessages(args);
      if (res.has_more || res.truncated) {
        res._pagination = {
          has_more: !!res.has_more,
          next_cursor: res.cursor,
          truncated: !!res.truncated,
          hint: res.truncated
            ? `Scan cap reached: results are newest-first and the RECENT window is complete; older messages beyond the cap were not examined this call. Page with cursor="${res.cursor}" — each call makes cap-worth of fresh progress, so paging always terminates at has_more=false.`
            : `More (older) messages exist — results are GLOBALLY newest-first. Call inbox_list again with cursor="${res.cursor}" to page older; has_more=false means you have seen every match.`,
        };
      }
      return res;
    },
  ...
  inbox_sweep: {
    ...
    description: 'Permanently delete expired inbox messages in a bounded cursor batch (cannot be undone), and maintain the ts-ordered listing index: missing index entries are backfilled while reindexing is incomplete, or on demand with reindex=true. A full failure-free walk (start with no cursor, page until has_more=false) marks reindexing complete.',
    requiredScope: 'memory:write',
    destructive: true,
    schema: { limit: z.number().int().min(1).max(1000).optional(), cursor: z.string().optional(), reindex: z.boolean().optional() },
    execute: sweepInbox,
  },
  artifact_put: {
  ...
  artifact_finalize: {
    ...
    execute: async (args, ctx = {}) => {
      ...
      // namespace (lib/artifact-store.js) — same rule as council_convene.
      if (!namespacesPermit(ctx.allowedNamespaces, 'qig')) {
        throw new Error('namespace-restricted credentials without "qig" access cannot finalize artifacts');
      }
      return (await finalizeArtifact(args)) ?? { error: 'not_found', ...args };
    },
  },
  artifact_manifest: {
  ...
  task_create: {
    ...
    execute: async (args, ctx = {}) => {
      // See app/api/tasks/route.js for the full rationale: the task runner's
      // LLM loop holds an unrestricted inbox_send, so a namespace-restricted
      // credential cannot be allowed to schedule a task at all.
      if (namespacesRestricted(ctx.allowedNamespaces)) {
        throw new Error('namespace-restricted credentials cannot create autonomous tasks');
      }
      const schedule = {
  ...
  task_update: {
    ...
    execute: async ({ id, ...patch }, ctx = {}) => {
      // Same rule as task_create: a restricted credential could otherwise
      // rewrite an EXISTING task's instruction (or re-activate a cancelled
      // one) to get an arbitrary-namespace inbox_send out of the unrestricted
      // cron-driven task runner — see app/api/tasks/[id]/route.js PATCH.
      if (namespacesRestricted(ctx.allowedNamespaces)) {
        throw new Error('namespace-restricted credentials cannot update autonomous tasks');
      }
  ...
  council_health: {
    title: 'Council Gateway Health Ping',
    description:
      'CHEAP but LIMITED pre-convene probe: ONE small model call through the AI Gateway (default helper model + its fallback chain, same ZDR filter and auth). What it CAN detect: total gateway death — auth failure, global outage, hard spend cap (RED with the unwrapped status/body when the SDK surfaces them). What it CANNOT detect: partial or route-correlated degradation — on 2026-07-22 five of six council seats died while a single cheap call exactly like this one succeeded, so GREEN would NOT have caught that incident. GREEN also does not certify the primary model: the gateway fallback chain may have served the reply (check servedBy/fallbackServed in the result). Treat RED as "do not convene"; treat GREEN as "not provably dead", never as a guarantee the convene will succeed.',
    // Deliberately write-scoped, and deliberately NO readOnlyHint here: the
    // probe spends real money on a model call, and readOnlyHint is derived
    // solely from READ_ONLY_TOOL_NAMES (annotationsFor) — an inline hint would
    // be dead code that contradicts the enforced scope and the manifest.
    requiredScope: 'memory:write',
    schema: {},
    execute: async () => {
      const { councilHealthPing } = await import('./council');
      const res = await councilHealthPing();
      return {
        ...res,
        verdict: res.ok
          ? 'GREEN — the gateway answered one cheap call. This rules out total gateway death only; it does NOT rule out the partial, per-route degradation that has previously killed 5 of 6 seats while this same probe succeeded.'
          : 'RED — do NOT convene; the spend would burn into this fault.',
      };
    },
  },
  ...
  council_convene: {
    ...
    execute: async (args, ctx = {}) => {
      ...
      // (lib/council.js) — not caller-controlled, so (unlike task_create) a
      // restriction can be checked against that one fixed namespace instead of
      // denied outright.
      if (!namespacesPermit(ctx.allowedNamespaces, 'qig')) {
        throw new Error('namespace-restricted credentials without "qig" access cannot convene the council');
      }
      ...
      // F2 durable job-state: write a 'pending' record at ACK so the memory_key
      // EXISTS the instant we return it — a caller can poll it even if the
      // background job never runs. conveneCouncil transitions pending -> running
      // -> completed; the catch below marks it failed.
      await putMemory(memoryKey, {
        category: 'council',
        content: JSON.stringify({
          status: 'pending',
          question: String(args.question ?? '').slice(0, 200),
          convener: args.convener,
          convened_at: convenedAt,
        }),
        source: args.convener ? `council convened by ${args.convener}` : 'council',
      }).catch((err) => console.error('[v0] council pending-mark failed:', err?.message));
      try {
        after(async () => {
          try {
            ...
            // Background failure is no longer console-only: persist a 'failed'
            // record so the memory_key reflects the dead job, not a stale 'pending'.
            await putMemory(memoryKey, {
              category: 'council',
              content: JSON.stringify({
                status: 'failed',
              ...
              source: args.convener ? `council convened by ${args.convener}` : 'council',
            }).catch((e) => console.error('[v0] council failed-mark failed:', e?.message));
          }
        });
      } catch (err) {
